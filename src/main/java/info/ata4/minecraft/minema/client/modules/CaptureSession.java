...
public class CaptureSession extends CaptureModule {
    ...
    protected void handleError(Throwable t) {
        ChatUtils.print("minema.error.label", EnumChatFormatting.RED);
        ...
        for (Throwable throwable : throwables) {
            ...
            ChatUtils.print(message, EnumChatFormatting.RED);
        }
    }
    ...
    public void captureFrame() {
        ...
        try {
            ...
            if (eventBus.post(new FrameCaptureEvent(time.getNumFrames(), fbc.getCaptureDimension(), fbc.getByteBuffer()))) {
                throw new RuntimeException("Frame capturing cancelled at frame " + time.getNumFrames());
            }
    ...
    private void setStream(IStream stream) {
        try {
            ReflectionHelper.setPrivateValue(Minecraft.class, MC, stream, PrivateFields.MINECRAFT_STREAM);
        } catch (Exception ex) {
        ...
        try {
            if (MC.theWorld != null && MC.thePlayer != null) {
                MC.theWorld.playSound(MC.thePlayer.posX, MC.thePlayer.posY, MC.thePlayer.posZ,
                        "mob.chicken.plop", 1, isEnabled() ? 2 : 1, false);
            }
        } catch (Exception ex) {
